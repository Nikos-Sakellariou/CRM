...
namespace CRMapp
{
    public partial class OrderFindDisNoteSupplier : Form
    {
        SqlConnectionStringBuilder con = new SqlConnectionStringBuilder();
        List<string> ls = new List<string>();
        Dictionary<string, string> dc = new Dictionary<string, string>();

        public OrderFindDisNoteSupplier(string SupplierId)
        {
        ...
        private void GetData(string SupplierId)
        {
            using (SqlConnection sqlcon = new SqlConnection(con.ConnectionString))
            {
                try
                {
                    ...
                    try
                    {
                        string query = "select DisNoteId,DisNoteSeries,DisNoteDate,Id from SupplierDisNote where SupplierId=@supid and Invoice='0' order by DisNoteDate";
