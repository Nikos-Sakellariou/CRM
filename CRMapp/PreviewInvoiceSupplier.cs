...
namespace CRMapp
{
    public partial class PreviewInvoiceSupplier : Form
    {
        SqlConnectionStringBuilder con = new SqlConnectionStringBuilder();
        Checks chk = new Checks();

        public PreviewInvoiceSupplier(string Id)
        {
            GC.Collect();
        ...
        private void GetData(string Id)
        {
            using (SqlConnection sqlcon = new SqlConnection(con.ConnectionString))
            {
                try
                {
                    ...
                    try
                    {
                        string query = "select SupplierId,Name,Afm,InvoiceId,InvoiceSeries,InvoiceDate,InvoicePayment,InvoiceDisc,InvoicePrice from SupplierInvoice a, Suppliers b where a.SupplierId=b.Id and a.Id=@id";
                        ...
                        string query2 = "select ProductId,SupplierDescr,ProductQuant,ProductPrice,ProductDisc,LongDescr from SupplierInvoiceProducts a, Products b where a.ProductId=b.Id and a.SupplierInvoiceId=@id";
