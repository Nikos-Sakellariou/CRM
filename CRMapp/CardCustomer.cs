...
namespace CRMapp
{
    ...
    public partial class CardCustomer : System.Windows.Forms.UserControl
    {
        ...
        private void RetrieveBtn_Click(object sender, EventArgs e)
        {
           ...
           //         try
            ...
            //            {
                            ...
                            {
                                query = "select Id,Name,Afm from Customers where UPPER(Name)=@parameter";
                                param = SearchNameTxt.Text;
                            }
                            else if (SearchAfmTxt.Text != "")
                            {

                                query = "select Id,Name,Afm from Customers where Afm=@parameter";
                            ...
                            else
                            {
                                ...
                                string query4 = @"select dbo.nvl(cast(Credit as decimal(18,2))-POS,0.00) from Customers b,(select dbo.nvl(sum(POS),0.00) as POS from
                                (select sum(cast(InvoicePrice as decimal(18,2))) as POS from CustomerInvoice where CustomerId = @id and InvoiceDate>=cast(@datefrom as Date)
                                union
                                select -sum(cast(DebitInvoicePrice as decimal(18,2))) from CustomerDebitInvoice where CustomerId = @id and DebitInvoiceDate>=cast(@datefrom as Date)
                                union
                                select -sum(cast(Price as decimal(18,2))) from CustomerBankDeposits where CustomerId = @id  and Date>=cast(@datefrom as Date)
                                union
                                select -sum(cast(ReceiptPrice as decimal(18,2))) from CustomerReceipt where CustomerId = @id  and ReceiptDate>=cast(@datefrom as Date)) a) c
								where b.Id = @id";
                                ...
                                if (dt4.Rows.Count==1)
                                {
                                    InitialCredit = Convert.ToDecimal(dt4.Rows[0][0].ToString());
                                }
                                ...
                                union
                                select Doc, '', Date, Price,'BankDeposits',Id from CustomerBankDeposits where CustomerId = @id
                                union
                                ...
                                lsview.Add(mtf);
                                CardCustomerLst.Items.Add(new ListViewItem(mtf));
                                for (int i = 0; (i < dt2.Rows.Count); i++)
                                {
                                    DataLst.Clear();
                                    ...
                                    {
                                        Reason = "ΕΙΣΠΡΑΞΗ ΜΕΤΡΗΤΩΝ/ΠΑΡΑΛΑΒΗ ΑΞΙΟΓΡΑΦΩΝ";
                                    ...
                                    PriceT = PriceT + Convert.ToDecimal(PriceX) - Convert.ToDecimal(PriceP);
                                    ...
                                    lsview.Add(data);
                                    CardCustomerLst.Items.Add(new ListViewItem(data));
                                    if (DataLst.Count()>0)
                                    {
                                        lsview.Add(Lbl);
                                        CardCustomerLst.Items.Add(new ListViewItem(Lbl));
                                        foreach (var item in DataLst)
                                        {
                                            lsview.Add(item);
                                            CardCustomerLst.Items.Add(new ListViewItem(item));
                                        }
                                    }
                                }
                                ...
                                lsview.Add(mtf2);
                                CardCustomerLst.Items.Add(new ListViewItem(mtf2));
                                BankDepositPanel.Enabled = true;
                            }
              //          }
             //           catch (Exception)
             //           {
             //               MessageBox.Show("Παρουσιάστηκε κάποιο μη αναμενόμενο σφάλμα στην αναζήτηση της καρτέλας του πελάτη. Παρακαλώ επικοινωνήστε με το διαχειριστή του συστήματος.");
              //          }
                        sqlcon.Close();
             //       }
             //       catch (Exception)
        ...
        private void BankDepositBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                System.Windows.MessageBox.Show("Θα πρέπει να επιλέξετε πελάτη.");
            }
            else
            {
                using (SqlConnection sqlcon = new SqlConnection(con.ConnectionString))
                {
                  ...
                  //      try
                  //      {
                            InsCmd1.CommandText = "insert into CustomerBankDeposits (Id, CustomerId,Doc, Date, Price) values((select dbo.nvl(Max(Id)+1,0) from dbo.CustomerBankDeposits), @supid, @doc, @date, @price)";
                            ...
                            InsTrans.Commit();
                            System.Windows.MessageBox.Show("Το έμβασμα του πελάτη καταχωρήθηκε με επιτυχία.");
        ...
        private void PrintDoc_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            if (CardCustomerLst.Items.Count > 0)
            {
                PrintDoc_Elements(e);
            }
        }
        ...
        private void PrintDoc_Elements(System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawString("Καρτέλα Κινήσεων Πελατών", new Font("Arial", 16, System.Drawing.FontStyle.Bold), Brushes.Black, 400, 35);
            ...
            e.Graphics.DrawString("Επωνυμία Πελάτη", new Font("Arial", 14, System.Drawing.FontStyle.Bold), Brushes.Black, 155, 85);
            ...
            e.Graphics.DrawString("ΧΡ. ΥΠΟΛ.", new Font("Arial", 13, System.Drawing.FontStyle.Regular), Brushes.Black, 965, 185);
