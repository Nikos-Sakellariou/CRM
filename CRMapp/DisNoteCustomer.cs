...
namespace CRMapp
{
    ...
    public partial class DisNoteCustomer : System.Windows.Forms.UserControl
    {
            ...
            using (SqlConnection sqlcon = new SqlConnection(con.ConnectionString))
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        string query = "select Id,Description,LongDescr,Quant,Unit from ProductsReserveView order by Id,Description";
        ...
        private void AddProdBtn_Click(object sender, EventArgs e)
        {
            ...
            System.Windows.Forms.TextBox PriceProdTxt = new System.Windows.Forms.TextBox();
            ...
            PriceProdTxt.Name = "PriceProdTxt" + i;
            PriceProdTxt.Location = new Point(PriceProdTxt1.Location.X, PriceProdTxt1.Location.Y + 21 * (i - 1));
            PriceProdTxt.Visible = PriceProdTxt1.Visible;
            PriceProdTxt.Size = PriceProdTxt1.Size;
            PriceProdTxt.Font = PriceProdTxt1.Font;
            ...
            this.ProductsPanel.Controls.Add(PriceProdTxt);
        ...
        private void SearchCustomerBtn_Click(object sender, EventArgs e)
        {
            ...
            FindCustomer.FormClosing += (object sender2, FormClosingEventArgs e2) =>
            {
                if (FindCustomer.ReturnCustomerId() != null)
                {
                    ...
                    if (FindCustomer.ReturnCustomerRetail()=="1")
                    {
                        PriceProdLbl.Visible = true;
                        PriceProdTxt1.Visible = true;
                        ReceiptNumDisNoteLbl.Visible = true;
                        ReceiptNumDisNoteTxt.Visible = true;
                        ReceiptDisNoteLbl.Visible = true;
                        ReceiptDisNoteTxt.Visible = true;
                    } 
                }
            };
        ...
        private void AddCustomerBtn_Click(object sender, EventArgs e)
        {
            ...
            AddCustomer.FormClosing += (object sender2, FormClosingEventArgs e2) =>
            {
                if (AddCustomer.ReturnCustomerId() != null)
                {
                    ...
                    if (AddCustomer.ReturnCustomerRetail() == "1")
                    {
                        PriceProdLbl.Visible = true;
                        PriceProdTxt1.Visible = true;
                        ReceiptNumDisNoteLbl.Visible = true;
        ...
        private void RemoveProdBtn1_Click(object sender, EventArgs e)
        {
            ...
            this.ProductsPanel.Controls.Remove(Controls.Find("PriceProdTxt" + i, true).First());
            i--;
            ProdItemsTxt.Text = i.ToString();
            proditems = i;
            this.ResumeLayout(false);
        ...
        private void CalcBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ReceiptNumDisNoteTxt.ReadOnly = true;
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            for (int i = 1; i <= Convert.ToInt16(ProdItemsTxt.Text); i++)
                            {
                                ...
                                string query = "select dbo.nvl(Price, 0),dbo.nvl(Disc, 0) from ProductsReserveView where Id = @prodid";
                                SqlDataAdapter SearchAdapt = new SqlDataAdapter(query, sqlcon);
                                SearchAdapt.SelectCommand.Parameters.AddWithValue("@prodid", ProductsPanel.Controls.Find("IdProdTxt" + i, true).First().Text);
                                DataTable dt = new DataTable();
                                SearchAdapt.Fill(dt);
                                if (dt.Rows.Count>0)
                                {
                                    prodprice = Math.Round(Convert.ToDecimal(dt.Rows[0][0].ToString()) * (1 - Convert.ToDecimal(dt.Rows[0][1].ToString()) / 100) * (1 + chk.Return_Vat(IdCustomerTxt.Text) / 100), 2);
                                    ProductsPanel.Controls.Find("PriceProdTxt" + i, true).First().Text = prodprice.ToString();
                                    MessageBox.Show(chk.Return_Vat(IdCustomerTxt.Text).ToString());
                                    sumpricereceipt += Convert.ToDecimal(ProductsPanel.Controls.Find("QuantProdTxt" + i, true).First().Text) * prodprice;
                                }
                            }
                            ReceiptDisNoteTxt.Text = sumpricereceipt.ToString();
                        }
                        catch (Exception)
                        {
                            MessageBox.Show("Παρουσιάστηκε κάποιο μη αναμενόμενο σφάλμα στην εύρεση της τιμής πώλησης. Παρακαλώ επικοινωνήστε με το διαχειριστή του συστήματος.");
                        }
                        sqlcon.Close();
                    }
                    catch (Exception)
        ...
        private void CorrectBtn_Click(object sender, EventArgs e)
        {
            ...
            ReceiptNumDisNoteTxt.ReadOnly = false;
                ...
                using (SqlConnection sqlcon = new SqlConnection(con.ConnectionString))
                {
                    try
                    {
                        sqlcon.Open();
                            ...
                            try
                            {
                                InsCmd1.CommandText = "insert into CustomerDisNote (Id, CustomerId, DisNoteId, DisNoteSeries, DisNoteDate, Invoice,Vehicle, Notes, Comments) values((select dbo.nvl(Max(Id)+1,0) from dbo.CustomerDisNote), @supid, @invid, @ser, @date,(select Retail from Customers where Id=@supid),@veh,@not,@comm)";
                                ...
                                for (int i = 1; i <= Convert.ToInt16(ProdItemsTxt.Text); i++)
                                {
                                    ...
                                    InsCmd3.CommandText = @"insert into CustomerDisNoteProducts (Id,CustomerDisNoteId,ProductId,ProductQuant,ProductSalePrice,ProductSaleDisc,ProductCostPrice) values ((select dbo.nvl(Max(Id)+1,0) from CustomerDisNoteProducts),(select dbo.nvl(Max(Id),0) from dbo.CustomerDisNote),@prodid,@prodquant,
                                    (select dbo.nvl(Price,0) from ProductsReserveView where Id=@prodid),(select dbo.nvl(Disc,0) from ProductsReserveView where Id=@prodid),(select dbo.nvl(OrderValue,0) from ProductsReserveView where Id=@prodid))";
                                ...
                                InsTrans.Commit();
                                MessageBox.Show("Το δελτίο αποστολής του πελάτη καταχωρήθηκε με επιτυχία.");
                                Do_Print();
                                ClearValues();
                                GetDataProd();
                            }
                            ...
                            {
                                MessageBox.Show(ex.GetType() + ": " + ex.Message + "\n Commit Exception \n Παρουσιάστηκε κάποιο μη αναμενόμενο σφάλμα στην καταχώρηση του δελτίου αποστολής του πελάτη. Παρακαλώ επικοινωνήστε με το διαχειριστή του συστήματος.");
                                try
                                {
                                    InsTrans.Rollback();
                                }
        ...
        private void ClearValues()
        {
            ...
            QuantProdTxt1.Text = "";
            PriceProdTxt1.Text = "";
            ReceiptDisNoteTxt.Text = "";
            ReceiptNumDisNoteTxt.Text = "";
            VehicleTxt.Text = "";
            NotesTxt.Text = "";
            CommentsTxt.Text = "";
