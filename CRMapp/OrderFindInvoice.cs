...
namespace CRMapp
{
    public partial class OrderFindInvoice : Form
    {
        SqlConnectionStringBuilder con = new SqlConnectionStringBuilder();
        List<string> ls = new List<string>();
        Dictionary<string, string> dc = new Dictionary<string, string>();

        public OrderFindInvoice(string SupplierId)
        {
        ...
        private void GetData(string SupplierId)
        {
            using (SqlConnection sqlcon = new SqlConnection(con.ConnectionString))
            {
                try
                {
                    ...
                    try
                    {
                        string query = "select InvoiceId,InvoiceSeries,InvoiceDate,Id from SupplierInvoice where SupplierId=@supid order by InvoiceDate";
        ...
        private void RetrieveBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Θα πρέπει να επιλέξετε ένα τουλάχιστον Τιμολόγιο του προμηθευτή.");
            }
            else
            {
