...
namespace CRMapp
{
    public partial class PreviewDebitInvoiceSupplier : Form
    {
        SqlConnectionStringBuilder con = new SqlConnectionStringBuilder();
        Checks chk = new Checks();

        public PreviewDebitInvoiceSupplier(string Id)
        {
            GC.Collect();
        ...
        private void GetData(string Id)
        {
            using (SqlConnection sqlcon = new SqlConnection(con.ConnectionString))
            {
                try
                {
                    ...
                    try
                    {
                        string query = "select SupplierId,Name,Afm,DebitInvoiceId,DebitInvoiceSeries,DebitInvoiceDate,DebitInvoicePayment,DebitInvoiceDisc,DebitInvoicePrice from SupplierDebitInvoice a, Suppliers b where a.SupplierId=b.Id and a.Id=@id";
                        ...
                        string query2 = "select ProductId,SupplierDescr,ProductQuant,ProductPrice,ProductDisc,LongDescr from SupplierDebitInvoiceProducts a, Products b where a.ProductId=b.Id and a.SupplierDebitInvoiceId=@id";
