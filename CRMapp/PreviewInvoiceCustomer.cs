...
namespace CRMapp
{
    public partial class PreviewInvoiceCustomer : Form
    {
        SqlConnectionStringBuilder con = new SqlConnectionStringBuilder();
        Checks chk = new Checks();

        public PreviewInvoiceCustomer(string Id)
        {
            GC.Collect();
        ...
        private void GetData(string Id)
        {
            using (SqlConnection sqlcon = new SqlConnection(con.ConnectionString))
            {
                try
                {
                    ...
                    try
                    {
                        string query = "select CustomerId,Name,Afm,InvoiceId,InvoiceSeries,InvoiceDate,InvoicePayment,InvoiceDisc,InvoicePrice from CustomerInvoice a, Customers b where a.CustomerId=b.Id and a.Id=@id";
                        ...
                        string query2 = "select ProductId,Description,ProductQuant,ProductPrice,ProductDisc,LongDescr from CustomerInvoiceProducts a, Products b where a.ProductId=b.Id and a.CustomerInvoiceId=@id";
